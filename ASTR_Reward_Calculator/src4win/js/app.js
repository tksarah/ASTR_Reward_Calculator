...
class ASTRRewardCalculator {
    ...
    calculateTotals() {
        ...
        const weightedAverageAstrPriceUsd = totalAstr > 0 ? totalRewardUSD / totalAstr : 0;
        const weightedAverageAstrPriceJpy = totalAstr > 0 ? totalRewardJPY / totalAstr : 0;
    ...
    exportToCSV() {
        ...
        lines.push('');
        lines.push('集計');
        lines.push(`総Claim数,${totals.totalCount}`);
        lines.push(`合計ASTR量,${totals.totalAstr.toFixed(8)}`);
        lines.push(`合計Fee(ASTR),${totals.totalFee.toFixed(8)}`);
        lines.push(`合計Fee(JPY),${totals.totalFeeJPY.toFixed(0)}`);
        lines.push(`総リワード(USD),${totals.totalRewardUSD.toFixed(2)}`);
        lines.push(`総リワード(JPY),${totals.totalRewardJPY.toFixed(0)}`);
        lines.push(`加重平均取得ASTR単価(USD/ASTR),${totals.weightedAverageAstrPriceUsd.toFixed(4)}`);
        lines.push(`加重平均取得ASTR単価(JPY/ASTR),${totals.weightedAverageAstrPriceJpy.toFixed(2)}`);
        lines.push(`最終損益額(JPY),${totals.netProfitJPY.toFixed(0)}`);

        const csv = lines.join('\r\n');
        this.downloadFile(csv, 'astr_reward_data.csv', 'text/csv;charset=utf-8;', { prependBom: true });
...
<body>
    ...
    buildReportBodyHtml({ entries, totals, monthlyData, chartImage, generatedAt, reportMetadata }) {
        ...
        const summaryCards = [
            ...
            ['加重平均取得ASTR単価（USD/ASTR）', `$${this.formatCurrency(totals.weightedAverageAstrPriceUsd, 4)}`],
            ['加重平均取得ASTR単価（JPY/ASTR）', `¥${this.formatCurrency(totals.weightedAverageAstrPriceJpy, 2)}`],
    ...
    downloadFile(content, filename, type, options = {}) {
        const { prependBom = false } = options;
        const blobParts = prependBom ? ['\uFEFF', content] : [content];
